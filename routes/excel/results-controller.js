...
async function getResultsInExcel(users, competition) {
    ...
    try {
        ...
        competition.events.forEach((event) => {
            ...
            const usersFiltered = filterUsersForEvent(
                users,
                competition,
                event.name
            );
            ...
            usersFiltered.forEach((user) => {
                ...
                if (!comp)
                    throw new Error(
                        "Competition not found, so filtering did not work."
                    );
                ...
                userEvent.rounds.forEach((round, index) => {
                    ...
                    solves.forEach((solve, index) => {
                        solves[index] = solve === 0 ? "DNF" : formatTime(solve);
                    });
...
function filterUsersForEvent(users, competition, eventName) {
    ...
    users.forEach((user) => {
        ...
        if (comp) {
            ...
            console.log(
                "Comp not found on user with username " + user.username
            );
        }
    });
