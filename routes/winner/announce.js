...
router.post("/announce", verifyToken, isAdmin, async (req, res) => {
    try {
        ...
        if (!winnerId) {
            return res
                .status(400)
                .json({ message: "Id pobjednika je obavezan." });
        }
        if (!competitionId) {
            return res
                .status(400)
                .json({ message: "Id netjacanja je obavezan." });
        }
        ...
        if (sameWinner) {
            ...
            return res
                .status(200)
                .json({ message: "Pobjednik uspješno izbrisan." });
        }
        ...
        return res
            .status(201)
            .json({ message: "Pobjednik uspješno objavljen." });
    } catch (error) {
...
function checkIfWinnerIsSame(allWinners, competitionId, group, winnerId) {
    const existingWinner = allWinners.find((winner) => {
        if (
            winner.competitionId.equals(competitionId) &&
            winner.group === group
        ) {
